...
const GLOBAL_KEY = "__chima_taskStatus";
...
function createTaskStatusExtension({ extensionAPI }) {
  ...
  const DONE_STATUS_CLEANUP_DELAYS_MS = [0, 50, 200, 800];
  ...
  const appliedColorVarNames = new Set();
  ...
  function clearStatusColorOverrides() {
    ...
    appliedColorVarNames.forEach((varName) => {
      try {
        style.removeProperty(varName);
      } catch (_) {}
    });
    appliedColorVarNames.clear();
  ...
  function applyStatusColorOverrides(overrides) {
    ...
    getRenderableStatusKeys().forEach((statusKey) => {
      ...
      if (values.bg) {
        try {
          style.setProperty(vars.bg, values.bg);
          appliedColorVarNames.add(vars.bg);
        } catch (_) {}
      }
      if (values.border) {
        try {
          style.setProperty(vars.border, values.border);
          appliedColorVarNames.add(vars.border);
        } catch (_) {}
      }
      if (values.fg) {
        try {
          style.setProperty(vars.fg, values.fg);
          appliedColorVarNames.add(vars.fg);
        } catch (_) {}
      }
...
.ts-status-choice-dot[data-task-status-key="${keySelector}"] {
  background-color: var(${vars.bg}) !important;
  color: var(${vars.fg}) !important;
  border-color: var(${vars.border}) !important;
}`);
    });
  ...
  const pendingDoneStatusCleanup = new Set();
  ...
  function isTodoCheckboxLikeTarget(target) {
    if (!target?.closest) return false;
    return Boolean(
      target.closest(".check-container") ||
        target.closest(".checkmark") ||
        target.closest(".rm-checkbox") ||
        target.closest("input[type='checkbox']")
    );
  }

  async function cleanupStatusOnDone(blockUid) {
    const blockString = getBlockString(blockUid);
    if (blockString === null) return { done: false, cleaned: false };

    if (!isDoneTask(blockString)) return { done: false, cleaned: false };

    const hasStatus = hasAnyStatusTag(blockString);
    if (!hasStatus) return { done: true, cleaned: false };

    const cleaned = removeStatusFromText(blockString);

    if (blockString === cleaned) return { done: true, cleaned: false };

    // If another Task Status operation is updating this block, retry shortly.
    if (pendingOperations.has(blockUid)) return { done: false, cleaned: false };

    pendingOperations.add(blockUid);
    try {
      await updateBlock(blockUid, cleaned);
    } finally {
      pendingOperations.delete(blockUid);
    }

    return { done: true, cleaned: true };
  }

  function scheduleStatusCleanupAfterDoneToggle(blockUid) {
    if (!blockUid) return;
    if (pendingDoneStatusCleanup.has(blockUid)) return;
    pendingDoneStatusCleanup.add(blockUid);

    let attempt = 0;

    const run = async () => {
      try {
        const res = await cleanupStatusOnDone(blockUid);
        if (res.done) {
          pendingDoneStatusCleanup.delete(blockUid);
          return;
        }
      } catch (err) {
        log("Done cleanup error:", err);
      }

      attempt += 1;
      if (attempt >= DONE_STATUS_CLEANUP_DELAYS_MS.length) {
        pendingDoneStatusCleanup.delete(blockUid);
        return;
      }

      window.setTimeout(run, DONE_STATUS_CLEANUP_DELAYS_MS[attempt]);
    };
  ...
  async function setBlockStatus(blockUid, statusKey, { blockStringOverride } = {}) {
    if (!blockUid) return;
    if (statusKey != null && !STATUSES[statusKey]?.active) return;
    if (pendingOperations.has(blockUid)) return;

    const blockString =
      typeof blockStringOverride === "string"
        ? blockStringOverride
        : getBlockString(blockUid);
    if (blockString === null) return;

    const newText =
      statusKey === null
        ? removeStatusFromText(blockString)
        : applyStatusToText(blockString, statusKey);

    if (newText === blockString) return;

    pendingOperations.add(blockUid);
    try {
      await updateBlock(blockUid, newText);
    } finally {
      pendingOperations.delete(blockUid);
    }
  }
  ...
  async function cycleBlockStatus(blockUid) {
    ...
    try {
      ...
      const newText =
        next === null
          ? removeStatusFromText(blockString)
          : applyStatusToText(blockString, next);
  ...
  function handleTodoDoneToggleClick(event) {
    const target = event.target;
    if (!isTodoCheckboxLikeTarget(target)) return;
    ...
    // If this click toggled TODO->DONE, remove the status tag.
    scheduleStatusCleanupAfterDoneToggle(blockUid);
  }
    ...
    window.roamAlphaAPI.ui.slashCommand.addCommand({
      ...
      callback: (context) => {
        void (async () => {
          ...
          let blockString = getLiveBlockInputValue(blockUid) ?? getBlockString(blockUid);
          if (blockString === null) return;

          blockString = getTextHelpers().removeSlashCommandFragment(blockString, indexes);

          await setBlockStatus(blockUid, statusKey, { blockStringOverride: blockString });
    ...
    const panelConfig = {
      ...
      settings: [
        ...
        },
        {
          ...
          name: "Help",
          description:
            "Stores status as a task-status/<Name> tag after TODO. Click tag to choose; Shift+click to remove. Marking DONE removes the status tag.",
          action: {
            type: "button",
            content: "Print Help",
            onClick: () => {
  ...
  function init() {
    // Best-effort cleanup of roam/js legacy globals (prevents duplicate handlers)
    try {
      window.taskStatusExtension?.cleanup?.();
    } catch (_) {}
    try {
      window.readyTaskExtension?.cleanup?.();
    } catch (_) {}
    ...
    window.addEventListener("click", handleTodoDoneToggleClick, true);
  ...
  function cleanup() {
    ...
    window.removeEventListener("click", handleTodoDoneToggleClick, true);
    pendingDoneStatusCleanup.clear();
